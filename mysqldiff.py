...
def main(oldConf, newConf, outFile):    
    ...
    oldTables = read_tables(oldCursor)  
    newTables = read_tables(newCursor)
    oldCursor.close()
    newCursor.close()             
    oldDB.close()
    newDB.close()

    diff = calc_diff(oldTables, newTables)
...
def read_tables(cursor):
    ...
    read_fields(tables, cursor)
    return tables

    
def read_fields(tables, cursor):
    for table in tables:
...
def calc_diff(oldTables, newTables):
    ...
    while oldIdx < len(oldTables) or newIdx < len(newTables):
        ...
        if oldIdx >= len(oldTables):
            ...
            tag_table(newTable, DiffType.ADDED)
        ...
        if newIdx >= len(newTables):
            ...
            tag_table(oldTable, DiffType.REMOVED)
        ...
        if oldTable.name == newTable.name:
            diffTable = calc_diff_table(oldTable, newTable)
...
def calc_diff_table(oldTable, newTable):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-old', dest='oldDb', required=True, 
        help='the connection to the old database')
    parser.add_argument('-new', dest='newDb', required=True,
        help='the connection to the new database')
